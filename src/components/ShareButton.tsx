...
import { Share2, X, Check, Copy, Link } from "lucide-react";
...
const socialPlatforms = [
  ...
  },
  {
    ...
    getUrl: (url: string, title: string, description?: string) => 
      `https://pinterest.com/pin/create/button/?url=${encodeURIComponent(url)}&description=${encodeURIComponent(description || title)}`,
  },
  {
    name: "LinkedIn",
    icon: (
        ...
        {isOpen && (
          <>
            ...
            <motion.div
              ...
              className="absolute right-0 top-full mt-2 z-50 bg-white rounded-2xl shadow-xl border border-gray-100 p-4 min-w-[280px]"
            >
              {/* Header */}
              ...
              <div className="flex flex-wrap gap-3 mb-4">
                {socialPlatforms.map((platform) => (
                  <button
                    ...
                    className={`${platform.color} text-white p-3 rounded-full transition-all duration-200 hover:scale-110 shadow-md`}
                    title={platform.name}
                  >
                    {platform.icon}
                  </button>
